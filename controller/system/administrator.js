...
exports.createNewAdministrator = function (req, res, next) {
    ...
    let administrator = {
        ...
        admin_gender: req.body.admin_gender === 1,
        ...
        admin_introduction: req.body.admin_introduction ||`我说后台用户`,
...
exports.updateAdministrator = function (req, res, next) {
    ...
    administratorAdminModel.updateAdministrator(administrator, function (err, result) {
        if (err)  {
            return next(err);
        }
        ...
        res.json({
            ...
            result: {
                ok: !!result,
                message: !!result ? '更新管理员成功！' : '更新管理员失败！',
            }
        });
    });
};
...
exports.updateSystemRoleBySystemId = function (req, res, next) {
    ...
    administratorAdminModel.updateAdministrator(condition, update,function (err, result) {
        if (err)  {
            return next(err);
        }
        res.json({
...
exports.getAdministratorDetailByToKen = function (req, res, next) {
    ...
    administratorAdminModel.getAdministratorDetailByToken(admin_token, function (err, result) {
        ...
        async.eachLimit(result.admin_roles, 1, function (item, ab) {
            ...
            // roleAdminModel.getPermByRoleIdList(item,(err,rvle)=>{
            //     if (err)  return ab(err);
            //     administrator.perms = administrator.perms.concat(rvle)
            //     ab()
            // });
...
exports.removeAdministrator = function (req, res, next) {
    ...
    let condition = {
        _id: {$exists: true, $eq: administratorId }
    };
    ...
    administratorAdminModel.updateAdministrator(condition,update, function (err, success) {
        if (err)   {
            return next(err);
        }
