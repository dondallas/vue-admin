        ...
        <el-row :gutter="20">
            ...
            <el-col :span="8">
                <el-card class="box-card">
                    ...
                    <el-input class="mgb-15" :placeholder="filterPlaceholderText"
                              v-model="filterMenuPermText"></el-input>
                    <el-tree @check-change="handleUpdateMenuPermForRole" show-checkbox ref="menuPermTreeRef"
                             :filter-node-method="filterNode"
                             :data="menuPermissionTree" :props="treeProps" node-key="pval" default-expand-all
            ...
            <el-col :span="8">
                <el-card class="box-card">
                    ...
                    <el-tree @check-change="handleUpdateApiPermForRole" show-checkbox ref="apiPermTreeRef"
                             :filter-node-method="filterNode"
                             :data="apiPermissionTree" :props="treeProps" node-key="pval" default-expand-all
                             :expand-on-click-node="false">
                            <span class="custom-tree-node" slot-scope="{node,data}">
                                <span>
                                    <span class="mgl-10">{{ data.pname }}</span>
...
<script>
    ...
    import {
        getPermList,
        getPermsListByRoleId,
        updateRolePermsByRoleId,
    ...
    export default {
        ...
        methods: {
            ...
            loadRolePerms() {
                ...
                getPermsListByRoleId({role_id: this.roleId}).then(res => {
                  console.log('res',res)
                    if (res.data.menuPvals.length > 0) {
                        this.roleMenuPvals = res.data.menuPvals;
                        let arr =res.data.newMenu;
                        let temp = [];
                        arr.map((v,i) => {
                          if(v.leaf) {
                            temp.push(v.pval)
                        ...
                        this.$refs.menuPermTreeRef.setCheckedKeys(temp);

                    }
                    if (res.data.apiPvals.length > 0) {
                        this.roleApiPvals = res.data.apiPvals;
            ...
            loadApiButtonPermissionTree() {
                getPermList().then(res => {
                    this.apiPermissionTree = res.data.list
                })
            },
            ...
            generateButtonPermissionTree(menuPermissionTreeCopy) {
                listBtnPermGroupByParent().then(res => {
            ...
            handleUpdateMenuPermForRole: debounce(function () {
                ...
                updateRolePermsByRoleId(data).then(res => {
                    if (res) {
