        ...
        <el-row :gutter="20">
            ...
            <el-col :span="8">
              ...
              菜单权限值建议使用前缀&nbsp;<el-tag size="mini" type="success">m:</el-tag>
                    ...
                    <el-tree ref="menuPermTreeRef" :filter-node-method="filterNode" :data="menuPermissionTree" :props="treeProps" node-key="pval" default-expand-all :expand-on-click-node="false">
            ...
            <el-col :span="8">
              ...
              按钮权限值建议使用前缀&nbsp;<el-tag size="mini" type="warning">b:</el-tag>
                    ...
                    <el-input class="mgb-15" :placeholder="filterPlaceholderText"
                              v-model="filterButtonPermText"></el-input>
            ...
            <span class="custom-tree-node" slot-scope="{ node, data }">
                ...
                <el-tooltip v-if="data.ptype==permType.MENU" style="margin-right: 80px;" content="添加按钮权限"
                            placement="top">
                ...
                <span v-if="data.ptype==permType.BUTTON">
                  <el-tooltip content="更新" placement="top">
                    <el-button class="update-btn" type="text" size="mini" icon="el-icon-edit"
                               @click="handleUpdateButton(data)"></el-button>
                  </el-tooltip>
                  <el-tooltip content="删除" placement="top">
                    <el-button class="delete-btn" type="text" size="mini" icon="el-icon-delete"
                               @click="handleDeleleButton(data)"></el-button>
                  </el-tooltip>
                </span>
            </span>
                    </el-tree>
            ...
            <el-col :span="8">
                    ...
                    <div slot="header">
                        <div class="title-box">
                            ...
                            <el-tooltip content="同步接口权限数据" placement="top">
                                <el-button style="font-size: 25px;" type="text" @click="handleSyncApiPermissionData"
              ...
              接口权限值建议使用前缀&nbsp;<el-tag size="mini">a:</el-tag>
                    ...
                    <el-input class="mgb-15" :placeholder="filterPlaceholderText"
                              v-model="filterApiPermText"></el-input>
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="30%">
            <el-form :rules="rules" ref="dataForm" :model="temp" label-position="top">
                ...
                <el-form-item label="权限值" prop="pval">
                    <el-input v-model="temp.pval" placeholder="已自动加上前缀'b:'，您只需填剩余部分，如：user:manage、user:add"
                              :disabled="dialogStatus=='updateButton'">
                ...
                <el-form-item label="权限类型" prop="ptype">
                    <el-select v-model="temp.ptype" v-if="dialogStatus=='updateButton'||dialogStatus=='addButton'"
                               :disabled="true">
        ...
        methods: {
            ...
            handleDeleleButton(data) {
                this.$confirm('您确定要永久删除该权限？', '提示', confirm).then(() => {
                    deletePerm({pval: data.pval}).then((res) => {
                        if(res){
                            this.initData();
            ...
            handleSyncMenuPermissionData() {
                ...
                syncMenuPerms({data:list}).then(res => {
                    this.initData();
